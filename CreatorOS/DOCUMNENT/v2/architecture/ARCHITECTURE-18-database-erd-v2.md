...
| status | TEXT | NOT NULL DEFAULT 'healthy' CHECK (status IN ('healthy','stale','missing','needs_reauthorization')) | Per-link source status (distinct from connection health) |
...
| auth_state | TEXT | NOT NULL DEFAULT 'disconnected' CHECK (auth_state IN ('authorizing','connected','healthy','syncing','degraded','stale','reauth_required','revoked','error','disconnected')) | Canonical connection health states; matches OpenAPI and TDD-01 |
| created_at | INTEGER | NOT NULL | Unix epoch ms |
| updated_at | INTEGER | NOT NULL | Unix epoch ms |
...
| state | TEXT | NOT NULL CHECK (state IN ('authorizing','healthy','syncing','degraded','stale','reauth_required','revoked','error','disconnected')) | Canonical connection health states; matches OpenAPI ConnectionHealth.state and TDD-01 local schema |
...
### 5.14 `connector_job_state`

| Column | Type | Description |
|---|---|---|
| id | UUID | Primary key |
| operation_id | UUID NOT NULL, FK to operations | Owning operation |
| queue_name | TEXT NOT NULL | BullMQ queue (e.g., provider-action-google-drive) |
| bullmq_job_id | TEXT NULLABLE | Queue-assigned job ID for correlation |
| state | TEXT NOT NULL DEFAULT 'pending' CHECK (state IN ('pending','claimed','running','succeeded','failed_retryable','failed_non_retriable','needs_user_action','dead_letter')) | Worker execution state |
| attempt_count | INTEGER NOT NULL DEFAULT 0 | Attempts so far |
| max_attempts | INTEGER NOT NULL | Configurable ceiling per job type |
| last_error_code | TEXT NULLABLE | Normalized error from last attempt |
| claimed_at | TIMESTAMPTZ NULLABLE | When worker claimed this job |
| completed_at | TIMESTAMPTZ NULLABLE | Terminal transition |
| created_at | TIMESTAMPTZ NOT NULL DEFAULT now() | Creation time |
| updated_at | TIMESTAMPTZ NOT NULL DEFAULT now() | Last change |

**Rules:** Postgres is the source of truth; BullMQ is delivery machinery only. The worker uses atomic SQL claim (`UPDATE ... WHERE status IN ('queued','failed_retryable') RETURNING *`). If no row returns, the job was already processed.

**Indexes:**

- `idx_connector_job_state_claim` on `(state, next_retry_at)` for worker claim
- `idx_connector_job_state_queue` on `(queue_name, state)` for per-provider monitoring
...
|---|---|---|
| id | UUID | Primary key |
| operation_id | UUID NULLABLE, FK to operations | Related operation when known |
| connection_id | UUID NULLABLE, FK to connection_account | Related connection when applicable |
| provider | TEXT NULLABLE | Provider identifier |
| reason | TEXT NOT NULL | Human-readable reason this case was opened (e.g., "unknown outcome after timeout") |
| severity | TEXT NOT NULL DEFAULT 'medium' CHECK (severity IN ('low','medium','high','critical')) | Escalation priority |
| status | TEXT NOT NULL DEFAULT 'open' CHECK (status IN ('open','investigating','resolved','dismissed')) | Case lifecycle |
| assigned_to | TEXT NULLABLE | Support/engineer assigned |
| resolution_note | TEXT NULLABLE | How it was resolved |
| created_at | TIMESTAMPTZ NOT NULL DEFAULT now() | When case opened |
| resolved_at | TIMESTAMPTZ NULLABLE | When closed |

**Rules:** Populated automatically by the connector worker when a job reaches an unknown outcome or DLQ. Support can manually create cases. Never auto-deleted; retained for audit.
