...
| DEC-033 | Backend job worker technology | ✅ Decided |
...
### DEC-033: Backend Job Worker Technology

**Final Decision:** Node.js/TypeScript running on managed containers.

**Rationale:** Every current Technical Design Document (TDD-03, TDD-04, TDD-05, TDD-06, TDD-07, TDD-08) already assumes Node.js/TypeScript for the connector worker, API BFF, and outbox relay. Supabase Edge Functions are insufficient for long-running provider jobs, token refresh coordination, multi-page pagination loops, per-connection rate-limit scheduling, and cursor advancement. Managed containers (e.g., Cloud Run, Fly.io, or equivalent) provide the required process durability, background execution time, and operational control for the connector service.
...
| 1.2 | 2026-08-23 | Ratified DEC-033 backend job worker technology as Node.js/TypeScript on managed containers. |
